-- Add PAUSED to client status options and ensure compute_client_status preserves manual overrides for PAUSED.
ALTER TYPE public.client_status ADD VALUE IF NOT EXISTS 'PAUSED';

-- Backfill seed for existing workspaces so PAUSED appears in the phase list
INSERT INTO public.client_status_phases (workspace_id, status_key, label, sort_order, color, auto_managed)
SELECT w.id, 'PAUSED', 'Paused', 65, 'muted', false
FROM public.workspaces w
ON CONFLICT (workspace_id, status_key) DO NOTHING;

-- Update seeding function so new workspaces also get PAUSED
...
BEGIN
  INSERT INTO public.client_status_phases (workspace_id, status_key, label, sort_order, color, auto_managed) VALUES
    ...
    (_workspace_id, 'PAUSED',               'Paused',               65,  'muted',       false),
...
-- Patch compute_client_status to also short-circuit when status is PAUSED (manual override).
...
BEGIN
  ...
  FOR _key, _spec IN SELECT * FROM jsonb_each(_kpis) LOOP
    ...
    ELSIF _key = 'lead_quality' THEN
      ...
      IF _credit_total IS NULL OR _credit_total = 0 THEN
        CONTINUE;
      END IF;
