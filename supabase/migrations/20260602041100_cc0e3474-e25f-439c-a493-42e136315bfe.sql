...
-- Fix NULL-merge bug + swap form_cvr for leads, redefine lead_quality as credit-score ≥640
...
BEGIN
  ...
  SELECT COALESCE(p.kpis, '{}'::jsonb), COALESCE(s.overrides, '{}'::jsonb),
         COALESCE(s.green_score_min, 80), COALESCE(s.yellow_score_min, 50)
  ...
  -- COALESCE each operand so a missing row never NULLs out the merge
  _kpis := COALESCE(_ws_preset, '{}'::jsonb)
        || COALESCE(_ws_overrides, '{}'::jsonb)
        || COALESCE(_client_preset, '{}'::jsonb)
        || COALESCE(_client_overrides, '{}'::jsonb);
...
BEGIN
  ...
  FOR _key, _spec IN SELECT * FROM jsonb_each(_kpis) LOOP
    ...
    ELSIF _key = 'lead_quality' THEN
      -- % of meta_leads (last 30d) whose "approx_credit_score?" answer is "above_640"
      SELECT
        COUNT(*) FILTER (
          WHERE EXISTS (
