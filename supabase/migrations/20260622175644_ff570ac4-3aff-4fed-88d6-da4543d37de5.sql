...
BEGIN
  ...
  -- Every (account, date) covered by ANY granular campaign row, so we never
  -- double-count by also adding the account-level daily fallback for it.
  granular_cover AS (
  ...
  -- Per-campaign spend within the window, attributed via the campaigns table.
  -- We include EVERY campaign (active or paused). Granular rows only exist
  -- for dates the campaign actually delivered, so a campaign paused before
  -- the window contributes zero rows here automatically.
  per_campaign_spend AS (
    ...
    JOIN public.campaigns c
      ON c.id = mgd.object_id
     AND c.client_id IS NOT NULL
    WHERE mgd.level = 'campaign'
      AND mgd.date >= _since_date
      AND c.client_id IN (SELECT client_id FROM targets)
  ),
  -- Account-level fallback only for (account, date) pairs with NO granular
  -- coverage at all (i.e., per-campaign breakdown hasn't synced yet).
  daily_fallback AS (
    SELECT
      a.client_id,
      mid.ad_account_id,
