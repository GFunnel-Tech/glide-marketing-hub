...
BEGIN
  ...
  -- Map every (account, date) covered by ANY granular campaign row
  -- so we don't fall back to the account-level daily row (which would
  -- silently include paused-campaign spend).
  granular_cover AS (
  ...
  -- Active-campaign-only spend, attributed to client via campaigns table.
  active_campaign_spend AS (
    SELECT
      c.client_id,
      mgd.ad_account_id,
      mgd.date,
  ...
  -- Daily fallback only for (account, date) tuples not covered by ANY
  -- granular campaign row (i.e. the account has no per-campaign data yet).
  daily_fallback AS (
    SELECT
      a.client_id,
      mid.ad_account_id,
  ...
  combined AS (
    SELECT * FROM active_campaign_spend
    UNION ALL
  ...
  spend_agg AS (
    SELECT client_id,
           ...
           COALESCE(SUM(0), 0)::numeric                    AS cpm_w_placeholder, -- unused
