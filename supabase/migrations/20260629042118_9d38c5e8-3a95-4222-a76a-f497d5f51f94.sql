...
BEGIN
  ...
  -- Hard-locked statuses always win
  ...
  -- Client-level launch within last 7 days → Learning
  IF _client.launched_at IS NOT NULL AND _client.launched_at > (now() - interval '7 days') THEN
    RETURN 'LEARNING';
  END IF;

  -- Campaign-level learning detection:
  --  (a) any ACTIVE campaign created/launched within the last 7 days, OR
  --  (b) any ad still in Meta's review/learning phase
  SELECT EXISTS (
    SELECT 1
    FROM public.campaigns ca
    WHERE ca.client_id = _client_id
      AND lower(ca.status) = 'active'
