-- Onboarding media capture: extends portal_onboarding, adds media-asset and
-- consent tables, and a private RLS-scoped storage bucket for the files.
--
-- Companion to /AUDIT.md. EMM-branded onboarding wizard surface in the app.

-- =============================================================================
-- 1. portal_onboarding extensions (kept additive so the legacy 4-step flow
-- continues to work; new wizard reads its own *_done flags).
-- =============================================================================
...
-- =============================================================================
-- 2. Asset table — one row per uploaded file.
-- =============================================================================
DO $$ BEGIN
...
-- Workspace members read everything in their workspace.
...
-- Workspace members with write role can write/update.
...
CREATE POLICY "Workspace writers manage media_assets" ON public.client_media_assets
  FOR ALL USING (can_write_workspace(auth.uid(), workspace_id))
  WITH CHECK (can_write_workspace(auth.uid(), workspace_id));

-- Portal users read + insert their own client's media (RLS scoped through
-- portal_users.user_id). They cannot delete and cannot touch other clients.
...
-- =============================================================================
-- 3. Consent log (append-only by convention — no UPDATE/DELETE policies).
-- =============================================================================
...
-- =============================================================================
-- 4. Private storage bucket for onboarding media.
-- Path convention: {client_id}/{kind}/{uuid}.{ext}
-- =============================================================================
INSERT INTO storage.buckets (id, name, public)
VALUES ('client-onboarding', 'client-onboarding', false)
ON CONFLICT (id) DO UPDATE SET public = false;

-- Helper SQL fragment (inlined): the path's first folder is the client id.
-- Anyone authenticated who is either (a) a portal user for that client, or
-- (b) a member of the client's workspace can read.

DROP POLICY IF EXISTS "Onboarding read: client portal user or workspace member"
  ON storage.objects;
    ...
    AND (
      EXISTS (
        ...
        WHERE pu.user_id = auth.uid()
          AND pu.client_id::text = split_part(name, '/', 1)
      )
      OR EXISTS (
...
DROP POLICY IF EXISTS "Onboarding write: client portal user or workspace member"
  ON storage.objects;
...
DROP POLICY IF EXISTS "Onboarding update: workspace writer"
  ON storage.objects;
...
DROP POLICY IF EXISTS "Onboarding delete: workspace admin"
  ON storage.objects;
...
-- =============================================================================
-- 5. Admin helper: status view + re-trigger RPCs.
-- =============================================================================

-- A single onboarding-status row per client (image count, voice presence,
-- counts of synced/failed assets, ready/processed counts) for the admin view.
...
-- Mark an asset (or a whole client's set) as ready for processing. Workspace
-- writers only — RLS on the underlying table enforces it too.
...
-- Reset Drive sync state on a client's assets so they can be retried by
-- the onboarding-drive-sync edge function. Workspace writers only.
